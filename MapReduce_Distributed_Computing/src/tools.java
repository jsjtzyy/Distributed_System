...
public class tools extends TimerTask{
    ...
    tools(Hashtable nodeTable, Hashtable actionTable, Hashtable deadTable , FileSystem f, Scheduler s) {
        //System.out.println("build function in tools");
        NodeTable = nodeTable;
        ActionTable = actionTable;
        DeadTable = deadTable;
        ...
        fileSystem = f;
        scheduler = s;
        //System.out.println("our scheduler " + scheduler == null);
        //logFile = new File(LogPath);
    }
    ...
    public void writeLog(int action, Node n) {
        ...
        try {
            synchronized(logFile) {
                fileOuts = new FileOutputStream(logFile, true);
                String out = "";
                out += new Date().getTime();
                if (action == 2) out += " add ";
                else if (action == 1) out += " fail ";
